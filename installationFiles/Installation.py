...
def get_BigRectangle(event,x,y,flags,param):
	...
	if event == cv2.EVENT_LBUTTONDOWN:
		...
		if len(listaAux2)==2:        
			...
			dictionary['High_Resolution']=listCorteAltaRes     
			#np.save(fileToWrite,lista)
			np.save(fileToWrite,dictionary)
			## file2.write("\n".listFinal)         
...
####--------º--------------------------------            
			...
			dictionary['Semaforo']=listaSemFinal
...
######################	
	...
	while True:
		...
		if keyPress == ord('y'):
			...
			if caja==1:
				dictionary['Partida']=listaAux1
			if caja==2:	
				dictionary['LLegada']=listaAux1
			if caja>2:
				numero=str(caja)
				nombre='caja'
				dictionary[caja+numero]=listaAux1
